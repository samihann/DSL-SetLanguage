...
object SetLangDSL:
  ...
  // Defining the main map function to implement the optimizing functions

  // The class Map Monadaic Function is definied with one map function. THe class accepts the expression as
  // the parameter and map function accepts the optimization function to be impleted on the expression as a input.
  case class MapMonadaicFunction (expressions: SetDeclarations){
    def map ( optimizationFunction: SetDeclarations => Any): Any =  {
      optimizationFunction(expressions)
    }
  }


  // A standard optimization function is defined in the language which the user can implement
  // on their set expressions.

// The optimozation is done using the properties of set theory.
  def optimizationFunction (expression: SetDeclarations) : Any =
    expression match {

      // A constant
      case SetDeclarations.Value(name) => SetDeclarations.Value(name)

      // A variable
      case SetDeclarations.Variable(name) =>
        val nameEval = SetDeclarations.Variable(name).eval()
        val output = nameEval match {
          // If evaluated expression is a variable
          case x: SetDeclarations.Variable => SetDeclarations.Variable(name)
          // If evaluated expression is a AION value
          case y: Any => SetDeclarations.Value(name)
        }
        output

      // If the expression is of type Intersection.
      // The optimization done is
      // Intersection(x,x) => x
      // Intersection(x,Set()) => Set()
      // Intersection(Set(), x) => Set()

      case SetDeclarations.Intersection(operation1,operation2) =>
        val op1 = operation1.eval()
        val op2 = operation2.eval()
        if(op1==op2){
          op1
        }
        else if(op1.asInstanceOf[scala.collection.mutable.Set[Any]].isEmpty){
          Set()
        }
        else if(op2.asInstanceOf[scala.collection.mutable.Set[Any]].isEmpty){
          Set()
        }
        else{
          SetDeclarations.Intersection(operation1,operation2).eval()
        }


      // If the expression is of type Difference.
      // The optimization done is
      // Difference(x,x) => Set()
      // Difference(x,Set()) => x
      // Difference(Set(), x) => Set()

      case SetDeclarations.Difference(operation1,operation2) =>
        val op1 = operation1.eval()
        val op2 = operation2.eval()
        if(op1==op2){
          ...
          SetDeclarations.Difference(operation1,operation2).eval()
        }


      // If the expression is of type SymmetricDifference.
      // The optimization done is
      // SymmetricDifference(x,x) => Set()
      // SymmetricDifference(x,Set()) => x
      // SymmetricDifference(Set(), y) => y

      case SetDeclarations.SymmetricDifference(operation1,operation2) =>
        val op1 = operation1.eval()
        val op2 = operation2.eval()
        if(op1==op2){
          scala.collection.mutable.Set()
          ...
          SetDeclarations.SymmetricDifference(operation1,operation2).eval()
        }


      // If the expression is of type Union.
      // The optimization done is
      // Union(x,x) => x
      // Union(x,Set()) => x
      // Union(Set(), y) => y

      case SetDeclarations.Union(operation1, operation2) =>
        val op1 = operation1.eval()
        val op2 = operation2.eval()
        if (op1 == op2 || op2.asInstanceOf[AnySetType].isEmpty) {
          op1
        }
        else if (op1.asInstanceOf[AnySetType].isEmpty) {
          op2
        }
        else if (op2.asInstanceOf[AnySetType].isEmpty) {
          op1
        }
        else {
          SetDeclarations.Union(operation1, operation2).eval()
        }
    }
      ...
      this match {
        ...
        case Variable(name) =>
          ...
          if (mapToUse.contains(name)) {
            ...
            // Partial Evaluation
            // Returning the Variable function back as the variable is not present in Map.
            val returnExp = Variable(name)
            return returnExp
          }
        ...
        case Union(operation1, operation2) =>
          ...
          if (CheckSet(operation1, operation2)) {
            ...
            val op1 = operation1.asInstanceOf[SetDeclarations].eval()
            val op2 = operation2.asInstanceOf[SetDeclarations].eval()
            if(op1.isInstanceOf[Variable] && op2.isInstanceOf[Variable]){
              val resultExp = Union(op1.asInstanceOf[SetDeclarations],op2.asInstanceOf[SetDeclarations])
              return resultExp
            } else if (op1.isInstanceOf[Variable] && !op2.isInstanceOf[Variable]){
              val value = SetDeclarations.Value(op2)
              val resultExp = Union(op1.asInstanceOf[SetDeclarations],value)
              return resultExp
            } else {
              val value = SetDeclarations.Value(op1)
              val resultExp = Union(value, op2.asInstanceOf[SetDeclarations])
              return resultExp
            }
          }
        ...
        case Intersection(set1, set2) =>
          if (CheckSet(set1, set2)) {
            ...
            val op1 = set1.asInstanceOf[SetDeclarations].eval()
            val op2 = set2.asInstanceOf[SetDeclarations].eval()
            if(op1.isInstanceOf[Variable] && op2.isInstanceOf[Variable]){
              val resultExp = Intersection(op1.asInstanceOf[SetDeclarations],op2.asInstanceOf[SetDeclarations])
              return resultExp
            } else if (op1.isInstanceOf[Variable] && !op2.isInstanceOf[Variable]){
              val value = SetDeclarations.Value(op2)
              val resultExp = Intersection(op1.asInstanceOf[SetDeclarations],value)
              return resultExp
            } else {
              val value = SetDeclarations.Value(op1)
              val resultExp = Intersection(value, op2.asInstanceOf[SetDeclarations])
              return resultExp
            }
          }
        ...
        case Difference(set1, set2) =>
          if (CheckSet(set1, set2)) {
            ...
            val op2 = set2.asInstanceOf[SetDeclarations].eval()
            if(op1.isInstanceOf[Variable] && op2.isInstanceOf[Variable]){
              val resultExp = Difference(op1.asInstanceOf[SetDeclarations],op2.asInstanceOf[SetDeclarations])
              return resultExp
            } else if (op1.isInstanceOf[Variable] && !op2.isInstanceOf[Variable]){
              val value = SetDeclarations.Value(op2)
              val resultExp = Difference(op1.asInstanceOf[SetDeclarations],value)
              return resultExp
            } else {
              val value = SetDeclarations.Value(op1)
              val resultExp = Difference(value, op2.asInstanceOf[SetDeclarations])
              return resultExp
            }
          }
        ...
        case SymmetricDifference(set1, set2) =>
          if (CheckSet(set1, set2)) {
            ...
            val op2 = set2.asInstanceOf[SetDeclarations].eval()
            if(op1.isInstanceOf[Variable] && op2.isInstanceOf[Variable]){
              val resultExp = SymmetricDifference(op1.asInstanceOf[SetDeclarations],op2.asInstanceOf[SetDeclarations])
              return resultExp
            } else if (op1.isInstanceOf[Variable] && !op2.isInstanceOf[Variable]){
              val value = SetDeclarations.Value(op2)
              val resultExp = SymmetricDifference(op1.asInstanceOf[SetDeclarations],value)
              return resultExp
            } else {
              val value = SetDeclarations.Value(op1)
              val resultExp = SymmetricDifference(value, op2.asInstanceOf[SetDeclarations])
              return resultExp
            }
          }
        ...
        case CartesianProduct(s1, s2) =>
          if (CheckSet(s1, s2)) {
            ...
            val op1 = s1.asInstanceOf[SetDeclarations].eval()
            val op2 = s2.asInstanceOf[SetDeclarations].eval()
            if(op1.isInstanceOf[Variable] && op2.isInstanceOf[Variable]){
              val resultExp = CartesianProduct(op1.asInstanceOf[SetDeclarations],op2.asInstanceOf[SetDeclarations])
              return resultExp
            } else if (op1.isInstanceOf[Variable] && !op2.isInstanceOf[Variable]){
              val value = SetDeclarations.Value(op2)
              val resultExp = CartesianProduct(op1.asInstanceOf[SetDeclarations],value)
              return resultExp
            } else {
              val value = SetDeclarations.Value(op1)
              val resultExp = CartesianProduct(value, op2.asInstanceOf[SetDeclarations])
              return resultExp
            }
          }
