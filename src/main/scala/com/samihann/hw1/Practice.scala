...
import com.samihann.hw1.SetLangDSL.{MapMonadaicFunction, optimizationFunction}
...
object Practice:
  ...
  @main def runPractice(): Unit = {
    ...
    println("Partial Evaluation 1: " + Union(Variable("partialSet1"), Variable("partialSet2")).eval())
    Assign("partialSet3", Value(1), Value(2)).eval()
    println("Partial Evaluation 2: "+ Union(Variable("partialSet4"), Variable("partialSet3")).eval() )

    Assign("opUnSet1", Value(1), Value(2)).eval()
    Assign("opUnSet2", Value(1), Value(2)).eval()
    println("Union Optimize : " + MapMonadaicFunction(Union(Variable("opUnSet1"), Variable("opUnSet2"))).map(optimizationFunction))


    Assign("opInSet3", Value(1), Value(2), Value(3)).eval()
    Assign("opInSet4", Value(mutable.HashSet())).eval()
    println("Intersetion Optimize : " + MapMonadaicFunction(Intersection(Variable("opInSet3"), Variable("opInSet4"))).map(optimizationFunction))


    Assign("sdfSet6", Value(1), Value(2), Value(3)).eval()
    Assign("sdfSet5", Value(Set())).eval()
    println("Difference Optimize : " + MapMonadaicFunction(Difference(Variable("sdfSet6"), Variable("sdfSet5"))).map(optimizationFunction))
